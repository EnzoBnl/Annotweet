...
import com.enzobnl.annotweet.systems.ModelBuilder
...
object TSACrossValidation {
  /**
    ...
    */
  def crossValidate(df: DataFrame, modelBuilders: Array[ModelBuilder], nChunks: Int = 10, metricName: String = "accuracy", verbose: Boolean = true):
  (TSACrossValidationResult, Map[String, Any], PipelineModel) = {
    if (nChunks <= 1) throw new IllegalArgumentException("nChunks must be > 1")
    // Split data in nChunks
    val dfShuffled = df.orderBy(rand())
    val dfsTest: Array[DataFrame] = dfShuffled.randomSplit((for (_ <- 1 to nChunks) yield 1.0).toList.toArray)
    ...
    var bestModel: (TSACrossValidationResult, Map[String, Any], PipelineModel) = (TSACrossValidationResult(0, 0), null, null)
    ...
    for (modelBuilder <- modelBuilders) {
      ...
      for (i <- 0 until nChunks) {
        ...
        model = Some(modelBuilder.train(dfsTrain(i)))
        ...
        val accuracy = evaluator.evaluate(model.get.transform(dfsTest(i)))
      ...
      if(res.mean > bestModel._1.mean) bestModel = (res, modelBuilder.options, model.get)
